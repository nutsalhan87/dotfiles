{ pkgs, nix-colorizer, python-pkg, flameshot-pkg }: let
  color_theme = let
    ...
    text.inactive = nix-colorizer.hex.darken text.active 0.5;
  in rec {
    bg = "#081014";
    primary = "#213E50";
    secondary = "#375e33";
...
in {
  ...
  in {
    ...
    config = {
      colors = let
        palette = bg: text: let
          bg_oklch = nix-colorizer.hex.to.oklch bg;
          bg_colorful = nix-colorizer.oklch.to.hex (bg_oklch // {
            L = bg_oklch.L + 0.1;
            C = bg_oklch.C + 0.05; 
          }); 
        in {
    ...
    bars = {
      default = {
        ...
        settings.theme.overrides = with color_theme; {
          ...
          separator_bg = bg;
          separator_fg = nix-colorizer.hex.darken text.inactive 0.1;
          separator = "  ";
        };
        icons = "awesome6";
        blocks = [
  ...
  systemd.user.services.picom.Service.ExecCondition = "/bin/sh 'test -z $WAYLAND_DISPLAY'";
